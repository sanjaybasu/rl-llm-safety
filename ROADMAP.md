...
| 2026-05-19 | Author list updated again: Bernardo Arevalo removed (no graduate degree). Final list: Basu, Patel, Sheth, Morgan, Batniji (n=5). |
...
| 2026-05-20 | **Deployment-grade positive findings.** `failure_mode_analysis.py`: 163/165 hazards (98.8%) caught by AT LEAST ONE architecture; only 2 irreducibly missed; 0 hazards universally agreed across all 10 architectures. `deployment_grade_analysis.py`: sens-floor of 0.85 achievable at spec 0.282 (LogReg+TF-IDF threshold-tuned); disagreement-stratified policy achieves sens 0.939 with clinician review of 1747/2000 (87% of messages); per-category best architecture has sens ≥0.80 for every category. Two deployment policies (Policy A high-recall screen + clinician confirmation; Policy B disagreement-stratified triage) articulated as actionable. |
| 2026-05-20 | **GitHub push.** 26 commits pushed to https://github.com/sanjaybasu/rl-llm-safety (auth swapped to personal sanjaybasu account). .gitignore extended to exclude per-message failure_mode CSV (contains message_ids). |
| 2026-05-20 | **Operational synthesis (Table 8)**: 4-component guardrail composition drawing on Shah JAMA 2024 (assurance labs), CHAI Blueprint 2023 (principles), and Sendak PLOS Digit Health 2024 (HAIP governance). Citation-verification agent confirmed the 4-component stack is the authors' operational synthesis, NOT a literal configuration any source prescribes — manuscript reframed accordingly for academic accuracy. None of 7 evaluated configurations reach clinical-grade; interpretation framed as "appears to require population-relevant extension" rather than "stack fails". All 9 strict concordance checks pass; 28 commits on main; pushed to GitHub. |
